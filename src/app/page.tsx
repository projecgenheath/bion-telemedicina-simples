...
export default function BionApp() {
  return (
    <BionProvider>
      <BionRoot />
    </BionProvider>
...
function BionRoot() {
  ...
  const { sessao, autenticado, carregando, sair } = useBion();
  ...
  // A view efetiva é derivada do estado de autenticação (sem efeitos):
  // deslogado → landing/login; logado com view de visitante → dashboard
  const viewReal: View = !autenticado
    ? view === "login"
      ? "login"
      : "landing"
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-sm font-bold text-muted-foreground">Carregando BION...</div>
        </div>
      </div>
    );
  }
  ...
  if (!autenticado) {
    if (view === "login")
      return <Login onEnter={() => setView("landing")} onSucesso={() => setView("dashboard")} />;
    return <Landing onEnter={() => setView("login")} />;
  }

  return (
    <Shell role={sessao.role} view={viewReal} setView={setView} onLogout={handleLogout}>
      {viewReal === "dashboard" && sessao.role === "paciente" && <PacienteDashboard go={setView} />}
      {viewReal === "dashboard" && sessao.role === "medico" && <MedicoDashboard go={setView} />}
      {viewReal === "dashboard" && sessao.role === "admin" && <AdminDashboard go={setView} />}
      {viewReal === "agendar" && (
        <AgendamentoFluxo
      ...
      {viewReal === "sala-espera" && <SalaEspera onEnter={() => setView("consulta")} />}
      {viewReal === "consulta" && (
        <Consulta
          role={sessao.role}
          onEnd={() => {
      ...
      {viewReal === "notificacoes" && <Notificacoes />}
      {viewReal === "consultas" && (
        <MinhasConsultas perfil={sessao.role === "medico" ? "medico" : "paciente"} />
      )}
      {viewReal === "medico-perfil" && <MedicoPerfilView />}
      {viewReal === "paciente-perfil" && <PacientePerfilView />}
      {viewReal === "historico" && <Historico />}
      {viewReal === "mensagens" && <Mensagens />}
      {viewReal === "lembretes" && <Lembretes />}
      {viewReal === "usuarios" && <Usuarios />}
      {viewReal === "prontuario" && <Prontuario />}
      {viewReal === "receitas" && (
        <Receitas perfil={sessao.role === "medico" ? "medico" : "paciente"} />
      )}
      {viewReal === "relatorios" && <Relatorios />}
      {viewReal === "suporte" && <ChamadosSuporte />}
      {viewReal === "bion-ia" && <BionIA />}
      {viewReal === "avaliacoes" && <MinhasAvaliacoes />}
      {viewReal === "auditoria" && <AuditTrail />}
      {viewReal === "admin-pacientes" && <AdminPacientes />}
      {viewReal === "admin-medicos" && <AdminMedicos />}
      {viewReal === "admin-agendamentos" && <AdminAgendamentos />}
      {viewReal === "medico-pacientes" && <MedicoPacientes />}
      {viewReal === "ajuda" && <Ajuda perfil={sessao.role} />}
      {viewReal === "privacidade" && sessao.role === "admin" && <PrivacidadeAdmin />}
      {viewReal === "privacidade" && sessao.role !== "admin" && <PrivacidadePaciente />}
...
/* ---------- Login (autenticação real via API) ---------- */
