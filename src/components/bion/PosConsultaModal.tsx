...
import {
  Check, Star, FileText, Download, Pill, Award, ArrowRight, ShieldCheck,
  Heart, Calendar, Sparkles, MessageSquare, Clock
} from "lucide-react";
      ...
      <div className="bg-card border rounded-3xl max-w-2xl w-full p-6 md:p-8 shadow-2xl space-y-6 my-auto">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            Atendimento com <strong>{medico}</strong> ({especialidade}) concluído. Seus documentos digitais assinados já estão prontos.
          </p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-muted/60 border space-y-3 text-xs">
          ...
          <div className="space-y-2">
            {docsRecentes.length === 0 ? (
              <p className="text-muted-foreground">Nenhum medicamento ou atestado precisou ser emitido nesta consulta.</p>
            ) : (
              docsRecentes.map((d) => (
                <div
                  key={d.id}
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-8 h-8 rounded-lg bg-primary-soft flex items-center justify-center text-primary shrink-0">
                      {d.tipo === "receita" ? <Pill className="w-4 h-4" /> : <Award className="w-4 h-4" />}
                    </div>
                    <div className="min-w-0">
                      <div className="font-bold text-foreground truncate">{d.titulo}</div>
                      <div className="text-muted-foreground text-[11px] truncate">Assinado digitalmente • {d.data}</div>
                    </div>
                  </div>
                  <button
                    onClick={() => gerarDocumentoPDF(d)}
          ...
          <div className="pt-2 border-t text-muted-foreground flex items-center gap-2">
            ...
            <span>Retorno sugerido pela médica: <strong>Em 30 dias</strong> para reavaliação clínica.</span>
          </div>
        </div>
        ...
        {!avaliado ? (
          <div className="space-y-4 pt-2 border-t">
            <div className="text-center">
              <div className="text-sm font-bold text-foreground">Como foi sua experiência de telemedicina?</div>
              <div className="text-xs text-muted-foreground mt-0.5">Sua avaliação ajuda outros pacientes e apoia o médico.</div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div className="p-2 rounded-xl bg-muted/40 border">
                <span className="text-muted-foreground block text-[11px]">Pontualidade</span>
                <span className="font-bold text-amber-500 flex items-center justify-center gap-0.5 mt-0.5">
                  <Star className="w-3 h-3 fill-amber-500" /> {pontualidade}.0
                </span>
              </div>
              <div className="p-2 rounded-xl bg-muted/40 border">
                <span className="text-muted-foreground block text-[11px]">Atenção Médica</span>
                <span className="font-bold text-amber-500 flex items-center justify-center gap-0.5 mt-0.5">
                  <Star className="w-3 h-3 fill-amber-500" /> {atencao}.0
                </span>
              </div>
              <div className="p-2 rounded-xl bg-muted/40 border">
                <span className="text-muted-foreground block text-[11px]">Clareza</span>
                <span className="font-bold text-amber-500 flex items-center justify-center gap-0.5 mt-0.5">
                  <Star className="w-3 h-3 fill-amber-500" /> {clareza}.0
                </span>
              </div>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-accent-soft text-center space-y-3">
            ...
            <div className="font-bold text-sm text-foreground">Muito obrigado pela sua avaliação!</div>
