...
function Visualizador({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={`Documento: ${titulo}`}>
      <div className="absolute inset-0 bg-[#0a1f44]/60 backdrop-blur-sm" onClick={onFechar} aria-hidden="true" />
      <div className="relative w-full max-w-lg max-h-[88vh] overflow-y-auto bp-coluna rounded-3xl bg-white dark:bg-[#0b1424] p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-3 mb-4">
          <h3 className="text-lg font-bold text-[#0a1f44] dark:text-[#f2f6fc]">{titulo}</h3>
          <div className="flex gap-2 shrink-0">
            ...
            <button onClick={onFechar} aria-label="Fechar documento" className="rounded-full p-2 bg-[#0a1f44]/5 dark:bg-white/10 text-[#0a1f44] dark:text-[#f2f6fc]">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        <div className="bp-impressao space-y-2 text-sm leading-relaxed text-[#0a1f44] dark:text-[#f2f6fc] whitespace-pre-line border-t border-[#0a1f44]/10 dark:border-white/10 pt-4">
          {corpo.map((p, i) => (
            <p key={i}>{p}</p>
          ))}
  ...
  return (
    <div className="px-5 py-6 space-y-4 bp-safe-top pb-24">
      ...
      <section aria-label="Documentos das consultas">
        <h2 className="text-2xl font-black text-[#0a1f44] dark:text-[#f2f6fc]">Documentos</h2>
        <p className="text-sm text-[#0a1f44]/60 dark:text-[#f2f6fc]/60 mb-4">
          Escolha uma consulta realizada para ver ou imprimir seus documentos.
        </p>

        {realizadas.length === 0 ? (
          <div className="bp-glass p-6 text-center text-sm text-[#0a1f44]/60 dark:text-[#f2f6fc]/60">
              ...
              <button
                ...
                className={`shrink-0 rounded-2xl px-4 py-3 text-left transition border ${
                  consultaSelecionada === c.id
                    ...
                    : "bp-glass border-transparent text-[#0a1f44] dark:text-[#f2f6fc]"
                }`}
              >
        ...
        {consultaSelecionada && (
          <div className="mt-4 space-y-2">
            {documentosDaConsulta(consultaSelecionada).length === 0 ? (
              <div className="bp-glass p-5 text-sm text-[#0a1f44]/60 dark:text-[#f2f6fc]/60">
              ...
              documentosDaConsulta(consultaSelecionada).map((d) => {
                ...
                return (
                  <div key={d.id} className="bp-glass p-4 flex items-center gap-3">
                    <div className="w-11 h-11 rounded-2xl bg-[#123e7d]/10 dark:bg-sky-400/10 text-[#123e7d] dark:text-sky-300 inline-flex items-center justify-center shrink-0">
                      <Icone className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-bold text-[#0a1f44] dark:text-[#f2f6fc]">{d.titulo}</div>
                      <div className="text-xs opacity-60">{d.data}</div>
                    </div>
                    <button
                      onClick={() => setVisualizando({ titulo: d.titulo, corpo: d.corpo })}
                      className="rounded-full border border-[#123e7d]/25 dark:border-sky-300/25 px-4 py-2 text-xs font-bold text-[#123e7d] dark:text-sky-300"
                    >
                      Ver
                    </button>
                    <button
                      ...
                      className="rounded-full p-2.5 bg-[#0a1f44]/5 dark:bg-white/10 text-[#0a1f44] dark:text-[#f2f6fc]"
                    >
      ...
      <section aria-label="Mensagens com médicos" className="pt-2">
        <h2 className="text-2xl font-black text-[#0a1f44] dark:text-[#f2f6fc]">Mensagens</h2>
        <p className="text-sm text-[#0a1f44]/60 dark:text-[#f2f6fc]/60 mb-4">
        ...
        {conversaAtiva && contatoAtivo ? (
          <div className="bp-glass overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3 border-b border-[#0a1f44]/8 dark:border-white/10">
              <button
                onClick={() => {
                  setConversaAtiva(null);
                }}
                aria-label="Voltar para contatos"
                className="rounded-full p-2 bg-[#0a1f44]/5 dark:bg-white/10 text-[#0a1f44] dark:text-[#f2f6fc]"
              >
                <ArrowLeft className="w-4 h-4" />
              </button>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold text-[#0a1f44] dark:text-[#f2f6fc] truncate">{contatoAtivo.nome}</div>
              ...
              {conversa.length === 0 ? (
                ...
                conversa.map((m) => (
                  <div key={m.id} className={`flex ${m.minha ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] px-4 py-2.5 text-sm ${m.minha ? "bp-acao rounded-3xl rounded-br-md" : "bg-white/60 dark:bg-white/10 text-[#0a1f44] dark:text-[#f2f6fc] rounded-3xl rounded-bl-md"}`}>
                      {m.texto}
            ...
            <div className="p-3 flex gap-2 border-t border-[#0a1f44]/8 dark:border-white/10">
              <input
                value={texto}
          ...
          <div className="space-y-2">
            {contatos.length === 0 ? (
              ...
              contatos.map((c) => (
                ...
                >
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#123e7d] to-[#0a1f44] text-white inline-flex items-center justify-center text-sm font-bold shrink-0">
                    {c.nome.split(" ").slice(-1)[0][0]}
                    {c.nome[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-[#0a1f44] dark:text-[#f2f6fc] truncate">{c.nome}</div>
                    <div className="text-xs opacity-60 flex items-center gap-1">
                      <MessageCircle className="w-3 h-3" /> {c.aberta ? "Conversa disponível" : "Janela de 30 dias encerrada"}
                    </div>
                  </div>
                  {c.naoLidas > 0 && (
                    <span className="shrink-0 min-w-6 h-6 px-2 rounded-full bg-[#123e7d] text-white text-xs font-bold inline-flex items-center justify-center">
                      {c.naoLidas}
                    </span>
                  )}
                </button>
