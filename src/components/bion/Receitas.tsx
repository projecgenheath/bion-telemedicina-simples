...
export function baixarDoc(d: Documento) {
  gerarDocumentoPDF(d);
  toast.success("PDF timbrado baixado", { description: d.titulo });
}
...
export function VisualizadorDoc({
  docs,
  index,
  onIndex,
  onClose,
    ...
    >
      <div className="bg-card w-full sm:max-w-2xl rounded-t-3xl sm:rounded-3xl max-h-[88vh] flex flex-col sm:my-6">
        <div className="flex items-center gap-3 p-4 border-b">
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-muted" aria-label="Fechar">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-4 border-t flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground">
            {index + 1} de {docs.length}
          </span>
          <div className="ml-auto flex items-center gap-2">
            <button
              onClick={() => baixarDocTxt(d)}
...
export function Receitas({ perfil }: { perfil: "paciente" | "medico" }) {
  ...
  // Reinicia a paginação quando os filtros mudam (ajuste de estado durante render,
  // padrão recomendado em vez de effect síncrono)
  const chaveFiltros = `${busca}|${fTipo}|${fMedico}|${de}|${ate}`;
  const [chaveAplicada, setChaveAplicada] = useState(chaveFiltros);
  if (chaveFiltros !== chaveAplicada) {
    setChaveAplicada(chaveFiltros);
    setLimite(PAGINA);
  }
  ...
  const filtrados = useMemo(() => {
    ...
    return documentos.filter((d) => {
      ...
      if (
        q &&
        ![d.titulo, d.conteudo, d.medicamento, d.observacoes, d.medico].some((v) =>
  ...
  const salvar = () => {
    ...
    if (tipo === "receita" && posologia.trim().length < 3)
      e["posologia"] = "Informe a posologia (ex.: 1 comprimido ao dia).";
    if (duracao.trim().length < 1)
      e["duracao"] =
        tipo === "receita"
          ? "Informe a duração do tratamento."
          : "Informe o período de afastamento.";
    if (conteudo.trim().length < 10)
      e["conteudo"] = "Descreva o documento com pelo menos 10 caracteres.";
    ...
    <div className="max-w-3xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {perfil === "medico"
              ? "Emita documentos para o paciente"
              : "Encontre, abra e baixe seus documentos"}
          </p>
        </div>
        {perfil === "medico" && (
          <button
        ...
        <div className="mt-6 bg-card border rounded-2xl p-5 space-y-3">
          ...
          <Field label="Título" erro={erros["titulo"]}>
            <input
              ...
              maxLength={80}
              placeholder={
                tipo === "receita" ? "Ex.: Losartana 50mg" : "Ex.: 2 dias de afastamento"
              }
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <Field label="Posologia" erro={erros["posologia"]}>
                <input
                  ...
                  maxLength={120}
                  placeholder="Ex.: 1 comprimido ao dia, pela manhã"
                  className="w-full px-4 py-3 rounded-xl border bg-background"
                />
              </Field>
            </div>
          ...
          <Field
            label={tipo === "receita" ? "Duração do tratamento" : "Período de afastamento"}
            erro={erros["duracao"]}
          >
            <input
                  aria-label="Duração"
      ...
      <div className="mt-6 bg-card border rounded-2xl p-4 space-y-3">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>
            Exibindo {Math.min(limite, filtrados.length)} de {filtrados.length} documento(s)
          </span>
          {filtroAtivo ? (
                ...
                <button
                  onClick={() => {
                    ...
                    registrarAudit({
                      ...
                      entidade: "documento",
                      entidadeId: d.id,
                      detalhes: `Visualização: ${d.titulo} (${d.tipo})`,
                    });
                  }}
                ...
                <button
                  onClick={() => {
                    ...
                    registrarAudit({
                      ...
                      entidade: "documento",
                      entidadeId: d.id,
                      detalhes: `Download PDF: ${d.titulo} (${d.tipo})`,
                    });
                  }}
                  className="p-2 rounded-lg hover:bg-muted"
                  title="Baixar"
