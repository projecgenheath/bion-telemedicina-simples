...
const iniciais: Pessoa[] = [
  {
    ...
    tipo: "medico",
    detalhe: "Clínica Geral • CRM 12345",
    situacao: "ativo",
  },
  {
    id: "u2",
  ...
  },
  {
    ...
    nome: "Dra. Julia Lima",
    tipo: "medico",
    detalhe: "Dermatologia • CRM 34567",
    situacao: "pendente",
  },
  {
    id: "u4",
    nome: "Marina Silva",
    tipo: "paciente",
    detalhe: "32 anos • marina@email.com",
    situacao: "ativo",
  },
  {
    id: "u5",
    nome: "João Pereira",
    tipo: "paciente",
    detalhe: "45 anos • joao@email.com",
    situacao: "ativo",
  },
  {
    id: "u6",
    nome: "Rita Souza",
    tipo: "paciente",
    detalhe: "28 anos • rita@email.com",
    situacao: "suspenso",
  },
];
...
export function Usuarios() {
  ...
  const mudar = (id: string, situacao: Situacao) => {
    ...
    const msgs: Record<Situacao, string> = {
      ativo: "Cadastro aprovado — acesso liberado",
      suspenso: "Usuário suspenso da plataforma",
      pendente: "Marcado como pendente de análise",
    };
    if (situacao === "ativo") toast.success(msgs.ativo);
    else if (situacao === "suspenso") toast.warning(msgs.suspenso);
    else toast.info(msgs.pendente);
  };
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="text-muted-foreground mt-1">
        Aprove médicos, gerencie pacientes e controle acessos
      </p>
      ...
      <div className="mt-4 space-y-2">
        {visiveis.map((p) => (
          <div
            key={p.id}
            className="bg-card border rounded-2xl p-4 flex items-center gap-4 flex-wrap"
          >
            ...
            <span className={`text-xs px-2.5 py-1 rounded-full capitalize ${cor[p.situacao]}`}>
              {p.situacao}
            </span>
            <div className="flex gap-2">
              {p.situacao === "pendente" && (
                <button
                  onClick={() => mudar(p.id, "ativo")}
